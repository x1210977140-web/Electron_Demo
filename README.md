...
```bash
# 构建 macOS 版本（仅在 macOS 上可用）
npm run build

# 构建 Windows 版本（仅在 Windows 上可用）
npm run build:win

# 构建 Linux 版本（仅在 Linux 上可用）
npm run build:linux
```

> **注意**: 跨平台构建需要在对应的操作系统上执行。如需构建所有平台的版本，请使用 GitHub Actions 自动化构建。
...
| 平台 | 文件格式 | 特性 | 下载位置 |
|------|---------|------|---------|
| macOS | .dmg | 标准安装包 | Release 页面 |
| Windows | .exe | NSIS 安装器（可选安装路径、桌面快捷方式） | Release 页面 |
| Linux | .AppImage | 便携式应用 | Release 页面 |
